Matching_Pennies_Game_MultiMemSize = function(n_agents, n_trials, function_give, noise, msize_range, bias, power){
    ...
    for (m in msize_range){
      ...
      for (t in 1:n_trials) {
        ...
        if (t <= m) {
          ...
          agent_take_choice <- MemAgent_Function(agent_give_choices[(t-m):(t-1)], m, bias, power)
        }
        ...
        payoff_temp = as.data.frame(cbind(agent_give_choices, agent_take_choices, agent_give_payoff_m, agent_take_payoff_m, trial, agent, m))
  ...
  colnames(payoff_df)[ncol(payoff_df)] <- "msize"
  return(payoff_df)
}